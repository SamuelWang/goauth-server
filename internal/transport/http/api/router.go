...
func RegisterRoutes(r *gin.Engine, cfg *config.Config, authService *auth.Service, userService *user.Service, providerService *provider.Service, clientService *client.Service) {
	apiGroup := r.Group("/api")
	apiGroup.Use(gin.Recovery(), gin.Logger(), middleware.ContextMiddleware(cfg))

	apiV1Group := apiGroup.Group("/v1")
	v1.RegisterRoutes(apiV1Group, authService, userService, providerService, clientService)
}
